/**
 * translations.js
 * Smart Order System — i18n module
 * Supports: he (RTL), en, de, nl (LTR)
 */
...
const translations = {
  ...
  en: {
    ...
    status_overdue: 'Overdue', status_debt: 'Open Balance',
    status_active: 'Active Today', status_inactive: 'Inactive', status_system: 'System Active',
    btn_confirm: 'Confirm Order', btn_edit: 'Edit Order', btn_send_update: 'Send Update',
    btn_voice: 'Voice Recording', btn_start_pick: 'Start Picking',
    btn_finish_pick: 'Order Picked', btn_missing_item: 'Missing Item',
    btn_delivered: 'Delivered', btn_navigate: 'Navigate (Waze)',
    btn_details: 'Details', btn_all_orders: 'All Orders', btn_invoices: 'Invoices',
    btn_edit_rest: 'Edit', btn_remind: 'Send Reminder', btn_mark_paid: 'Mark as Paid',
    btn_manage: 'Manage Assignment', btn_download_pdf: 'Download PDF',
    btn_payment_info: 'Payment Details', btn_new_order: 'New Order',
    btn_open: 'Open →', btn_to_collect: 'Collections',
    whatsapp_auto: 'Automatic WhatsApp', whatsapp_greeting: 'Hello! We received your order',
    whatsapp_detail: 'Here are the details:', your_order: 'Your Order',
    pickup_requested: 'Requested Pickup', tomorrow_at: 'Tomorrow',
    ...
    or_separator: '— or —', my_orders: 'My Orders', personal_area: 'Personal Area',
    track_order: 'Track Order', realtime_status: 'Real-time Status', filter_all: 'All',
    ...
    invoice_label: 'Invoice', item_col: 'Item', qty_col: 'Qty', price_col: 'Price',
    total_col: 'Total', total_to_pay: 'Total Due', total_paid_label: 'Total Paid',
    ...
    currently_picking_label: 'Currently Picking', waiting_to_pick: 'Awaiting Picking',
  ...
  de: {
    ...
    status_overdue: 'Überfällig', status_debt: 'Offener Saldo',
    status_active: 'Heute aktiv', status_inactive: 'Inaktiv', status_system: 'System aktiv',
    btn_confirm: 'Bestellen bestätigen', btn_edit: 'Bestellung bearbeiten',
    btn_send_update: 'Änderung senden', btn_voice: 'Sprachaufnahme',
    btn_start_pick: 'Kommissionierung starten', btn_finish_pick: 'Bestellung kommissioniert',
    ...
    whatsapp_detail: 'Hier sind die Details:', your_order: 'Ihre Bestellung',
    pickup_requested: 'Gewünschte Abholzeit', tomorrow_at: 'Morgen',
    ...
    api_note: 'Preise werden aus der Buchhaltungssoftware synchronisiert',
  ...
  nl: {
    ...
    status_overdue: 'Achterstallig', status_debt: 'Openstaand saldo',
    status_active: 'Vandaag actief', status_inactive: 'Inactief', status_system: 'Systeem actief',
    ...
    whatsapp_detail: 'Hier zijn de details:', your_order: 'Uw bestelling',
    pickup_requested: 'Gewenste ophaaltijd', tomorrow_at: 'Morgen',
    ...
    system_active: 'Systeem actief',
    phases_title: 'Systeem ontwikkelingsroadmap',
    phases_sub: '6 fasen · Elke fase is onafhankelijk',
    tag_base: 'Basis', tag_simple: 'Eenvoudig te gebruiken', tag_ops: 'Operaties',
...
/**
 * getLang() — read stored language or default
 */
function getLang() {
...
/**
 * switchLanguage(lang) — main API: update UI, persist, dispatch event
 */
function switchLanguage(lang) {
  ...
  // 2. Update document direction + lang attribute
  const dir = tr.dir;
  ...
  // 4. Update data-i18n-placeholder separately (inputs with visible label)
  ...
  // 6. Dispatch custom event so page-specific JS can react
...
/**
 * injectLangSwitcherStyles() — inject once into <head>
 */
function injectLangSwitcherStyles() {
  ...
  s.textContent = `
    ...
    .lang-switcher [data-lang-btn]:hover {
      background: #e5e7eb;
      color: #111827;
    }
    .lang-switcher [data-lang-btn].ls-active {
      background: #fff;
      color: #1d4ed8;
    ...
    [dir=ltr] .flip-icon { transform: scaleX(-1); }
  `;
  document.head.appendChild(s);
}

/**
 * buildPageHeader(config) — inject a consistent top bar with lang switcher
 * config: { titleKey, subtitleKey, badgeKey }  — all optional
 */
function buildPageHeader(config = {}) {
  ...
  if (config.titleKey || config.subtitleKey) {
  ...
  // Mark active
  document.querySelectorAll('[data-lang-btn]').forEach(btn => {
    btn.classList.toggle('ls-active', btn.getAttribute('data-lang-btn') === getLang());
  });
}
